...
public static class ChatMessageServerPatch
{
    static bool Prefix(
    ...
    )
    {
        ...
        if (messageTrimmed.StartsWith(ModContext.Config.ChatTrigger))
        {
            ...
            if (parts.Length > 0)
            {
                ...
                {
                    var ctx = new CommandContext(args, _cInfo.entityId);
        ...
        var chatMessageEvent = new ChatMessageEvent(_cInfo.entityId, _chatType, mainName, _msg, _recipientEntityIds, _bbMode);
            ...
            if (_recipientEntityIds != null)
            {
                foreach (var recipientEntityId in chatMessageEvent.RecipientEntityIds)
                    SingletonMonoBehaviour<ConnectionManager>.Instance.Clients.ForEntityId(recipientEntityId)
                        ?.SendPackage(NetPackageManager.GetPackage<NetPackageChat>().Setup(chatMessageEvent.ChatType,
                            -1, message, null, EMessageSender.None, chatMessageEvent.BBMode));
            }
            else
                SingletonMonoBehaviour<ConnectionManager>.Instance.SendPackage(
                    NetPackageManager.GetPackage<NetPackageChat>().Setup(chatMessageEvent.ChatType, -1, message,
                        null, EMessageSender.None, chatMessageEvent.BBMode), true);
        }
        else
